...
### 2026-09-10（修用户实测 5 问题 + 数据目录统一到桌面）
- **用户反馈**：「右键没有任何选项了」「三横跟随延迟很高」「没有动作」「拖动没有任何反应很生硬」「鼠标移入没有显示输入栏了」
- **① 右键菜单空白（严重）**
  - **根因**：`_build_3d_menu` 里用 `_mate3d_sections()` 从 `_build_mate3d_menu()` 的返回值里抽子菜单。但那个 `full` 是**局部变量**，函数返回后被 Python **GC 回收**，挂在它上面的子菜单一起失效 → 实测报 `RuntimeError: wrapped C/C++ object of type QMenu has been deleted`，菜单渲染成空白
  - **修复**：改为**自建**全部子菜单（`_fill_3d_role_menu` / `_fill_3d_expr_menu` / `_fill_3d_action_menu` / `_fill_3d_size_menu` / `_fill_3d_window_menu` / `_fill_3d_hide_menu` / `_fill_3d_fun_menu`），并把子菜单存到 `self._xxx_sub` 防 GC。彻底删掉 `_mate3d_sections`
- **② 三横浮层跟随延迟**
  - **根因**：轮询定时器 400ms —— 拖动 3D 窗口后三横要过 0.4 秒才挪过去
  - **修复**：降到 **33ms**（约 30fps）；另在拖动时通过 `win.overlay_sync` 回调**即时**同步（零轮询延迟）
- **③ 没有动作**
  - **根因**：待机动画幅度只有 `0.004m`（4 毫米）、且只是整体平移，肉眼完全看不出
  - **修复**：放大到**呼吸 ±1.2cm**（周期 3.1s）+ 摇摆 ±1.2° + **手臂轻摆 ±3.5°** + **每 7 秒歪头一次**（平滑淡入淡出）；`relaxArms` 记录 `armBase` 作为摆动基准，避免每帧覆盖姿势
- **④ 拖动没反应、很生硬**
  - **根因（两层）**：
    ① 只重写了 `QWidget.mousePressEvent`，但窗口里铺满 `QWebEngineView`，鼠标事件全被 web 视图吃掉，**根本传不到 QWidget**
    ② JS 侧 `pyBridge.dragWindow(dx,dy)` 和 Python 侧**同时在搬窗口**，两套互相打架 → 抖动、迟滞
  - **修复**：Python 侧装 `eventFilter`（装在 view 上）统一接管拖动，按屏幕坐标增量移动 + 屏幕内夹紧；**关掉 JS 侧拖动**（JS 只保留视线跟随）
- **⑤ 鼠标移入不显示输入栏（两个根因）**
  - **根因 A**：3D 模式下 **2D 桌宠窗口是隐藏的**，而输入栏的展开是靠 `PetWindow.enterEvent` 触发的 → 永远不触发。修复：`Web3DPetWindow` 新增 `hoverChanged` 信号，在 `eventFilter` 里检测进出并转发给 `pet._on_web3d_hover`
  - **根因 B**：输入栏位置用 `self._pet.frameGeometry()` 算，3D 下 2D 窗口隐藏 → 位置无效（输入栏跑到屏幕别处）。修复：新增 `_GeoProxy` 几何代理 + `_set_geo_anchor()`，进 3D 时把 `ai._pet` / `chat._pet` 的锚点**永久**换成 3D 窗口，退回 2D 时还原。
    **⚠ 为什么不能用「临时替换」**：输入栏跟随靠定时器每 100ms 跑 `_follow_pet()`，临时替换在调用瞬间有效、定时器跑时补丁已还原 → 位置纹丝不动（实测踩过）。另外换锚点必须清 `_follow_offset`（它缓存了相对旧锚点的偏移）
  - 另：原来要求 `ai.enabled()` 才弹输入栏，现改为**AI 未启用也弹**（给交互反馈，不静默无反应）
- **⑥ 数据目录统一到桌面（避免两份数据打架）**
  - **背景**：以前源码运行读【脚本目录】、打包运行读【exe目录/卡丘简易桌宠数据】，两份数据并存 → 用户改了一份程序读另一份（"设置没生效""数据不见了"）
  - **修复**：`base_dir()` 统一 —— 源码运行也优先用**桌面「卡丘简易桌宠数据」**（该目录存在时）；支持 `PET_DATA_DIR` 环境变量显式指定
  - `_seed_default_assets()` 改为**源码运行也执行**：首次运行把内置 `assets`（24 角色立绘/人设/世界书/语音）播种到桌面数据目录，已存在的子项不覆盖（保护用户导入的内容）
  - 数据迁移时**以桌面版为基准合并配置**：桌面版有 `ai.enabled/base_url` 等用户实际配置，项目版独有的键补入（实测保住了 AI 配置 22 个子键）
- **新增测试**：`verify_fixes.py` **19/19** ✅（针对本次 5 个问题逐条验证）
- **回归**：`web3d_integration_test.py` 30/30 ✅；`panel_ctxmenu_test.py` 17/17 ✅

### 2026-09-10（设置面板集成 3D：形象角色里的 3D 条目 + 角色右键「切换 3D 形象」）
- **用户反馈**：「你要的是设置面板「形象角色」里新增一个 3D 选项 这个可以保留不过要这样子，有3D模型的相应角色右键可以冒出来切换3D形象」
- **① 设置面板「形象角色」列表新增 3D 条目**
  - 列表**第一项**固定为 `🎭 3D 米雪儿（立体）`（青色加粗），后面跟分组标题 `— 2D 图片角色 —` 再列图片角色
  - 当前处于 3D 时该项显示 `←当前`，且顶部「当前角色」标签同步显示 3D
  - **实现关键**：3D 不是图片角色（不在 `pet.roles` 里），所以用哨兵值 `_ROLE_3D = '__3d__'` 占位；`_on_role_clicked` 见到哨兵就走 3D 切换，不走 `load_role` 换图
  - 从 3D 点回任意图片角色 → 自动先退 3D 再换图
- **② 角色右键菜单「切换 3D 形象」（本次重点）**
  - 角色列表启用 `CustomContextMenu`，右键弹出菜单
  - **有对应 3D 模型的角色** → 显示 `🎭 切换 3D 形象（米雪儿）`（已在 3D 时追加 `● 使用中`）
  - **没有 3D 模型的角色** → 显示灰色 `（该角色暂无 3D 形象）`，避免用户以为功能坏了
  - 另外提供 `使用该角色（2D）` 与 `📂 打开角色文件夹`
  - 新增 `web3d_pet.ROLE_TO_3D` 映射表（2D 角色名 → 3D 模型名）+ `role_3d_model()` / `roles_with_3d()`；
    **以后给别的角色做 3D 模型，只需在 `ROLE_TO_3D` 加一行，右键菜单自动出现该功能**
  - `role_3d_model()` 会二次校验模型文件真的存在，避免映射写了但模型被删
  - 新增 `pet.switch_to_3d_for_role(role)`：先把该 2D 角色设为当前角色（**语音来源 / AI 人设跟着走**），再进 3D，等窗口就绪后热切换加载对应模型
- **验证**：`panel_ctxmenu_test.py` **17/17** ✅（新增）；`panel_3d_test.py` **14/14** ✅（新增）；
  `web3d_integration_test.py` **30/30** 回归 ✅
...
- **问题① 头脚被裁（相机取景算错）**
  - **根因**：`loadModel` 用 `camera.position.z = size.y * 1.25` 构图。FOV 25° 下可见高度只有 `2 × 1.25 × size.y × tan(12.5°) = 0.554 × size.y` → **只装得下全身 55%**；对焦点又取 `size.y * 0.62 = 0.99m`，视野落在 y∈[0.55, 1.44] → **正好切掉大腿以下和下巴以上**
  - **实测数据**：模型真实高度 1.6m，相机距离仅 2.0m，`fitH=3.61 / fitW=4.07`，实际需要 `4.56m`（含 12% 余量）→ 当前只装得下 **43.9%**
  - **修复**：新增 `fitCamera(vrm)` —— 由包围盒反推距离 `max(fitH, fitW) × margin`，**再做迭代收敛**：把包围盒 8 角投影到 NDC，按超出量放大距离（因为角色有 z 深度 −0.42~0.20，透视下近端角会投影到更外侧，纯解析式算不准，实测脚底会溢出 −0.1%）
  - 顺带 `onResize` 时重算：窄窗口要退更远才装得下（取景距离依赖宽高比）
  - **效果**：角色占窗口高度 55% → **89.5%**，上下空白 5.8% / 2.1%，头脚完整 ✅
- **问题② 默认展开双臂（T-pose）**
  - **根因**：这个 VRM（及多数 MMD 转来的模型）rest pose 就是 **T-pose**：实测肩 Y=1.2929、手 Y=1.2929（**完全水平**），整体宽 1.353m 而身高仅 1.6m
  - **修复**：新增 `relaxArms(vrm)` —— 上臂绕 Z 轴旋转 ±1.45 rad（≈83°）让手自然垂到身侧，并绕 X 轴 0.06 略往前避免穿裙。**只在检测到 T-pose 时才摆**（判据：`|手Y − 肩Y| > 身高×10%` 则判定已自然站姿、跳过），避免破坏本来就正常的模型
  - 实测角度表（`diag_pose.py`）：`rz=0` 手 Y=1.293 宽 1.353；`rz=±1.4` 手 Y=1.708/0.878；取 ±1.45 后**宽 1.353 → 0.568** ✅
- **问题③ 表情菜单在 3D 里点了没反应**
  - **根因**：表情菜单**统一**走 `_mate_link.blends_dict()`，那是**外置 Mate-Engine 的日文表情表**（にこり/笑い/あ/…）；而内置引擎的表情是 **VRM 标准名**（happy/blink/aa/…）→ 名字对不上，点了落空
  - **修复**：`_build_mate3d_menu` 按后端分流 —— 内置引擎读 VRM 表情表（`Web3DPetWindow.expressions()`，由 JS 在模型加载后回报），外置仍读 Mate 表；菜单显示「😊 开心 happy」这种中英对照，未列出的自动收进「更多表情」子菜单（保证功能不丢）
- **问题④（测试中发现）眨眼残留导致表情回不到原位**
  - **现象**：关掉自动眨眼后，顶点仍残留 `nz=1718` 偏移（应为 0）
  - **根因**：`setBlink()` 里 `delete morphDriver.state['blink']` **绕过了 MorphDriver 的 `_dirty` 标记** → `apply()` 因「状态没变」提前 return → 顶点卡在上一次眨眼的形状
  - **修复**：改走有 dirty 语义的 `morphDriver.set('blink', w)`（权重 0 由 apply 内部忽略）
- **问题⑤（测试中发现）快照取样时机造成假阴性**
  - **现象**：同一组表情，两次测试结论完全相反（`aa` 一次 5.3%、一次 0.0%），且「顶点没变却有 4.5% 差异」
  - **根因**：`toDataURL` 只画一帧就取图，可能拿到**「属性已改但还没上传绘制」的旧帧**
  - **修复**：`__snapshot` **连画两帧**。实测噪声基线 = **0px**（连拍 6 帧 md5 完全相同，渲染是确定性的），确认问题全在取样时机
  - 另加 `window.__setIdleMotion(bool)`：待机呼吸/轻摆会污染像素差分（实测基准漂移 18673px），自动化测试需先冻结
- **新增诊断工具**：`diag_pose.py`（包围盒/取景/上臂角度扫描）、`verify_framing.py`（头脚是否完整可见）、`verify_all_models.py`（4 模型取景）、`expr_menu_test.py`（表情菜单端到端 13 项）、`verify_expr_visible.py`（真实页面表情可见性）、`ascii_view.py` / `silhouette_view.py` / `face_zoom.py` / `ascii_compare.py`（**把图转字符画，让纯文字模型也能"看"画面** —— 视觉通道不可用时用这个确认观感）
- **顺带修正**：`shot_pet.py` 遗留 `vrm.scene.rotation.y = Math.PI`（会拍后脑勺）
- **验证**：`verify_framing.py` 8/8 ✅；`verify_all_models.py` **4/4 模型取景完整**（米雪儿/阿尔蒂娜/祖姆/拉兹莉）✅；`expr_menu_test.py` 13/13 ✅；`web3d_integration_test.py` **30/30 回归通过** ✅；轮廓图确认双臂垂放、双腿完整到底部 ✅
...
- **语音清单（用户原句，平常口语）**：①门快开了，赶紧走 ②先不要踩检查点，等等后面的人 ③先阻击一下，保护断后的人 ④所有人和螳螂距离一个冲刺距离 ⑤我来开门，其他人垫后 ⑥机枪先走，到点再架枪
- **生成**（`_voice_probe/gen_quick_voices.py`，直连本地 8080 `breeze-tts-clone`，无 PyQt 依赖）：
  - 24 角色 × 6 条 = 144 条，每条用该角色 `references/refs_en/<英文名>.wav+txt` 克隆 + `instructions` 指令：女＝「语气急切带催促感，语速稍快，声音清亮上扬，像战斗中提醒队友，干脆利落有精神」；男＝「语气沉稳严肃，语速适中偏慢，像指挥官下达命令，冷静果断，字字清晰，不带多余情绪」
  - 朗读文本 = 用户原句 + 句末标点（标点=停顿）；文件名 = 语音名（用户原句，如 `门快开了，赶紧走.mp3`）；wav 合成后 ffmpeg 转 24k 单声道 96k mp3
  - 输出双写：数据目录 + 源码 `assets/characters/<阵营>/<角色>/`（新机器首启由 `_seed_default_assets` 自动带上）
  - 实测 144/144 成功、0 失败；音频 2.8~5.3s；断点续跑（已存在跳过）、失败重试 2 次
- **绑定**（`_voice_probe/gen_quick_bind.py`）：数据目录 `pet_config.json` → `audio_hotkeys` 新增 144 条 `Num1`~`Num6`（key=`<阵营>/<角色>/<语音名>.mp3`），保留原有 2 条 `__common__` 绑定
- **零代码改动**：文件放角色目录顶层，`_hotkey_slot_audio` 的 `rsplit('/',1)` 前缀匹配正好命中（`<角色>/<文件名>`）；点按触发音 24 角色都有集中 `trigger_voice/卡拉彼丘.mp3`，优先于角色目录扫描，不会被新语音抢走；快捷语音同时出现在设置面板角色音频列表（可试听/重绑，附带价值）
- **验证**：模拟 `_hotkey_slot_audio` 匹配逻辑全量断言——24 角色 Num1~6 绑定齐全、文件存在、匹配错误 0、common 绑定保留 ✅；双写一致性抽查 ✅；当前运行 exe 无需重打包（纯数据+配置）
- 涉及：`assets/characters/`（144 新 mp3）、`pet_config.json`（数据目录）、`_voice_probe/gen_quick_voices.py`、`_voice_probe/gen_quick_bind.py`、`DEVELOPMENT.md`
- git：本提交
